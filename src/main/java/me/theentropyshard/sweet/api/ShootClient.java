...
public class ShootClient extends WebSocketListener {
    ...
    public void onMessage(@NotNull WebSocket webSocket, @NotNull String text) {
        ...
        switch (event.getType()) {
            ...
            case GUILD_DELETE -> {
                ServerGuildDeleteEvent guildDeleteEvent = this.gson.fromJson(event.getData(), ServerGuildDeleteEvent.class);

                this.listeners.forEach(listener -> listener.onGuildDelete(guildDeleteEvent));
            }

            case ROLE_CREATE -> {
                ServerRoleCreateEvent roleCreateEvent = this.gson.fromJson(event.getData(), ServerRoleCreateEvent.class);

                this.listeners.forEach(listener -> listener.onRoleCreate(roleCreateEvent));
            }
            ...
            case RELATIONSHIP_CREATE -> {
                ServerRelationshipCreateEvent relationshipCreateEvent = this.gson.fromJson(event.getData(), ServerRelationshipCreateEvent.class);

                this.listeners.forEach(listener -> listener.onRelationshipCreate(relationshipCreateEvent));
            }
